		...
		for (int j = 0; j < nSignedAttsCount; j++)
		{
			...
			{ //V_ASN1_OCTET_STRING
				...
				if (NULL != av)
				{
					...
					{
                        std::snprintf(buf, sizeof(buf), "%02x", (uint8_t)av->value.octet_string->data[m]);
						strSHASum += buf;
					}
...
bool ZSignAsset::Init()
{
    ...
    BIO *bioPKey = BIO_new_mem_buf(m_strCertData.data(), static_cast<int>(m_strCertData.size()));
    if (bioPKey != NULL)
    {
        evpPKey = PEM_read_bio_PrivateKey(bioPKey, NULL, 0, (void *)m_passData.c_str());
        if (evpPKey == NULL)
        {
            BIO_reset(bioPKey);
            ...
            if (evpPKey == NULL)
            {
                ...
                if (p12 != NULL)
                {
                    if (!PKCS12_parse(p12, m_passData.c_str(), &evpPKey, &x509Cert, NULL))
                    {
                        CMSError(); // Your error handling
                    }
                    PKCS12_free(p12);
                }
            }
	...
	if (NULL == x509Cert && !m_strCertData.empty())
	{
        BIO *bioCert = BIO_new_mem_buf(m_strCertData.data(), static_cast<int>(m_strCertData.size()));
        if (bioCert != nullptr)
        {
