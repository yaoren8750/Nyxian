...
string JValue::asString() const
{
	...
	{
        char buf[256];
        std::snprintf(buf, sizeof(buf), "%" PRId64, m_Value.vInt64);
        return buf;
	}
	break;
	case E_FLOAT:
	{
		char buf[256];
        std::snprintf(buf, sizeof(buf), "%lf", m_Value.vFloat);
		return buf;
	}
	break;
	case E_ARRAY:
...
void JReader::error(string &strmsg) const
{
	...
	char msg[64];
    std::snprintf(msg, sizeof(msg), "Error: Line %d, Column %d, ", row, int(m_pErr - plast) + 1);
	strmsg += msg + m_strErr + "\n";
}
...
void PWriter::FastWriteValue(const JValue &pval, string &strdoc, string &strindent)
{
	...
	else if (pval.isInt())
	{
		...
		strdoc += "<integer>";
		char temp[32] = {0};
        std::snprintf(temp, sizeof(temp), "%" PRId64, pval.asInt64());
		strdoc += temp;
		strdoc += "</integer>\n";
	}
	else if (pval.isFloat())
	{
		...
		else
		{
			...
			if (floor(v) == v)
			{
                std::snprintf(temp, sizeof(temp), "%" PRId64, (int64_t)v);
			}
			else
			{
                std::snprintf(temp, sizeof(temp), "%.15lf", v);
			}
			strdoc += temp;
		}
