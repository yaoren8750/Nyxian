...
static void overwriteExecPath(NSString *bundlePath) {
    ...
    const char *newPath = [bundlePath stringByAppendingPathComponent:@"Nyxian"].UTF8String;
    size_t maxLen = rnd64(strlen(path), 8);
    size_t newLen = strlen(newPath);
...
NSString* invokeAppMain(NSString *selectedApp, NSString *selectedContainer, int argc, char *argv[]) {
    ...
    NSString *bundlePath = [NSString stringWithFormat:@"%@/bundle", NSTemporaryDirectory()];
    ...
    // MARK: As a test i disabled this check
    ...
    /*if(!guestAppInfo) {
        ...
        // MARK: Skip overwriting the correct test bundle path
        //bundlePath = [NSString stringWithFormat:@"%@/Applications/%@", appGroupFolder.path, selectedApp];
        guestAppInfo = [NSDictionary dictionaryWithContentsOfFile:[NSString stringWithFormat:@"%@/LCAppInfo.plist", bundlePath]];
        isSharedBundle = true;
    }*/
    
    // MARK: Disabled this check for now
    /*if(!guestAppInfo) {
    ...
    NSLog(@"LDELC Loader: app bundle allocated at %p", appBundle);
    
    if(!appBundle) {
        return @"App not found";
    }
    
    // MARK: We dont need this for now
    ...
    *_CFGetProgname() = NSProcessInfo.processInfo.processName.UTF8String;
    ...
    printf("[*] loaded dylib %s at: %p | %s\n", appExecPath, appHandle, dlerror());
    
    void *mainSymbol = dlsym(appHandle, "main");
    ...
    int ret;
    // MARK: LindChain will never run arm32, bcuz why would someone want to do that
    //if(!is32bit) {
