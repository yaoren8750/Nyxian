...
std::string FormatSize(int64_t size, int64_t base)
{
    ...
    const int64_t TB = base * base * base * base;
    const int64_t GB = base * base * base;
    const int64_t MB = base * base;
    const int64_t KB = base;

    oss << std::fixed << std::setprecision(2);

    if (size >= TB)
    {
        fsize = static_cast<double>(size) / TB;
        oss << fsize << " TB";
    }
    else if (size >= GB)
    {
        fsize = static_cast<double>(size) / GB;
        oss << fsize << " GB";
    }
    else if (size >= MB)
    {
        fsize = static_cast<double>(size) / MB;
        oss << fsize << " MB";
    }
    else if (size >= KB)
    {
        fsize = static_cast<double>(size) / KB;
        oss << fsize << " KB";
    }
    else
    {
...
bool SHA1Text(const std::string &strData, std::string &strOutput)
{
    ...
    {
        std::snprintf(buf, sizeof(buf), "%02x", static_cast<uint8_t>(strSHASum[i]));
        strOutput += buf;
    }
...
{
    PrintSHASum(prefix, (const uint8_t *)strSHASum.data(), (int)strSHASum.size(), suffix);
}
...
void ZLog::Print(int nLevel, const char *szLog)
{
    ...
    {
        write(ls_getfd(), szLog, strlen(szLog));
    }
}
...
{
    write(ls_getfd(), "\033[31m", 5);
    write(ls_getfd(), szLog, strlen(szLog));
    write(ls_getfd(), "\033[0m", 4);
    return false;
}
